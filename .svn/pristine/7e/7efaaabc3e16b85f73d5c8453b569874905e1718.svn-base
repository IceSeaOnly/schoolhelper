...
<!--点击确认订单，弹出付款详情-->
<form action="/user/go_to_pay.do?orderKey=${order.orderKey}" method="post">
...
<%--<!--使用细则弹出层-->--%>
<%--<div class="statement" id="statement" style="z-index:5000">--%>
...
<script>
    ...
    var ali = privilege.getElementsByTagName("li");
    var aul = privilege.getElementsByTagName("ul")[0];
    var li = ali[0].offsetHeight + "px";//得到单个li的高度
    x = ali[0].offsetHeight * ali.length + "px";	//得到所有li的高度和
    var i = 0;
    aul.onclick = function () {//改变箭头方向
        if (i == 0) {
            $(".ul").animate({height: x});
            $(".arrows").attr("src", "images/select_1.jpg");
            i = 1
        } else {
            $(".ul").animate({height: li});
            $(".arrows").attr("src", "images/select.jpg");
            i = 0
        }//ul的高度等于li的高度和

    }
    $("li").click(function () {
        $(this).css("background-color", "#f9f9f9").siblings().css("background-color", "#fff");//点击时改变li的背景颜色
        $(".div_li").html($(this).html());//替换li内的内容
        $(".div_li h3").css("color", "#65646A");
        $(".div_li span").css("color", "#E60002")
    })
    //支付成功与重新支付
    $(".succeed").hide();
    $(".black").hide();
